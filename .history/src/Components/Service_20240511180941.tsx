export const Service = () => {
  return (
    <div className="font-montserrat space-y-16 pb-40">
      <h2 className="text-gray text-4xl font-medium tracking-wide max-lg:text-center">
        How my <span className="text-white"> service process</span> looks like
      </h2>
      <div className="xl:grid xl:grid-cols-3 flex flex-col w-full sm:w-2/3 xl:w-full max-xl:m-auto gap-8">
        {ServiceLList.map((service, index) => {
          return (
            <div
              key={`service numéro ${index} , name  :${service.name}`}
              className="bg-blackGray smpy-10 py-7 px-4 sm:px-8 rounded-3xl flex gap-5 flex-col items-start"
            >
